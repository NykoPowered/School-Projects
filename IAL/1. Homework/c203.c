...
*/
	...
	return (index + 1) % QUEUE_SIZE;	//ak index > QUEUE_SIZE tak sa vracia index zaciatocneho prvku
...
*/
	...
	return (q->f_index == q->b_index);	//ak indexy f a b ukazuju na rovnaky prvok tak fronta je prazdna a vracia 1. inac 0
}
...
*/
	...
	return nextIndex(q->b_index) == q->f_index;	//ak index f == b tak fronta je plna a vrati 1 inac 0
}
...
*/
    ...
    *c = q->arr[q->f_index];        //na adresu kam ukazuje *c ulozi prvy prvok
}
...
*/
    ...
    q->f_index = nextIndex(q->f_index); //indexom prveho prvku bude prvok za f_index
}
...
*/
    ...
    queueFront(q,c);        //c ukazuje na prvy prvok
    queueRemove(q);         //index prveho prvku bude nasledujuci index
}
...
*/
    ...
    q->arr[q->b_index] = c;         //vlozi znak c na prvu volnu poziciu
    q->b_index = nextIndex(q->b_index);     //b_index bude nastaveny na dalsiu volnu poziciu
}
